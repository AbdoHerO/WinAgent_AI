...
def extract_text_from_pdf(pdf_path):
    ...
    try:
        ...
        for i, page in enumerate(doc):
            # Update progress for very large documents
...
def get_best_available_model():
    ...
    try:
        ...
        # Extract model names based on the structure of the response
        available_models = []
...
def load_document_state():
    """Loads a previously saved document state."""
    ...
    try:
        ...
        def process_thread():
            try:
                ...
                root.after(0, lambda: chat_window.insert(tk.END,
                                                         f"❌ Error processing loaded document: {str(e)}\n"))
...
# Save document state button
save_btn = tk.Button(
    top_frame,
    text="💾 Save Doc",
    command=save_document_state,
    font=("Arial", 11),
    bg="#9C27B0",
    fg="white",
    padx=10
...
# Load document state button
load_btn = tk.Button(
    top_frame,
    text="📤 Load Doc",
    command=load_document_state,
    font=("Arial", 11),
    bg="#9C27B0",
    fg="white",
    padx=10
